#' HK weather - Downloading relative humidity information from https://data.gov.hk/en/help/api-spec
#'
...
#' @export
#'
#' @examples hk_load_rhum_csv()
hk_load_rhum_csv = function(time = weather2::tool_datetime(end = Sys.time(), by = "10 min", duration = "7 day"), lan = "en",
  ...
  URL = tibble::tibble(time = time) %>%
    ...
    dplyr::distinct()
  #Start to download
  weather2::sys.load_file(data = URL, attempt = attempt, title = "Relative Humidity (HKO)",
