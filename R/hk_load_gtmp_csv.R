#' HK weather - Downloading grass temperature data from https://data.gov.hk/en/help/api-spec
#'
...
hk_load_gtmp_csv = function(time = weather2::tool_datetime(end = Sys.time(), by = "10 min", duration = "7 day"), lan = "en",
  ...
  URL = tibble::tibble(time = time) %>%
    ...
    dplyr::distinct()
    #Start to download
    weather2::sys.load_file(data = URL, attempt = attempt, title = "Grass Temperature Information (HKO)",
