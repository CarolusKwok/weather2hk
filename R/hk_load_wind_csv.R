#' HK weather - Downloading wind and gust information from https://data.gov.hk/en/help/api-spec
#'
...
#' @export
#'
#' @examples hk_load_wind_csv()
hk_load_wind_csv = function(time = weather2::tool_datetime(end = Sys.time(), by = "10 min", duration = "7 day"), lan = "en",
  ...
  URL = tibble::tibble(time = time) %>%
    ...
    dplyr::distinct()
  #Start to download
  weather2::sys_load_file(data = URL, attempt = attempt, title = "Wind and Gust (HKO)",
